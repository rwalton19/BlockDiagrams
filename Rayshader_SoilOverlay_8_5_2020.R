...
thematic_shp <- readOGR('C:/geodata/ProjectData/WI033BD1GSM.shp')
# Insepct to see what column has MUSYM symbol
head(thematic_shp)
# Assign column with map unit label
mu.col <- "GSM"

# Load DEM in TIFF Format
elev <- raster('C:/geodata/ProjectData/WI033BD1LIDAR.tif')
...
extent <- readOGR(dsn='C:/geodata/ProjectData/WI033BD1Extent2.shp')
...
elmat %>%
  ...
  plot_3d(elmat, zscale=1, fov=0.8, theta=10, zoom=0.6, phi=45, windowsize = c(1000,800), lineantialias = TRUE, water=FALSE, waterdepth=0, wateralpha=0.75, watercolor = "lightblue")
...
for(i in 1:360) {
  ...
  render_snapshot(filename = sprintf("test%i.png", i), 
                  title_text = "Insert Title Here",
                  title_bar_color = "#008080", title_color = "white", title_bar_alpha = 1)
}

rgl::rgl.close()

av::av_encode_video(sprintf("test%d.png",seq(1,1440,by=1)), framerate = 30,output = "test.mp4")
