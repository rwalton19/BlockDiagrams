...
#10 meter DEM
elev = raster::raster("C:/geodata/elevation/usgs_10m_wtm1.tif")

#90 meter DEM
#elev = raster::raster("C:/geodata/elevation/90m_DEM.tif")
...
#res(elev_template) <- c(90, 90) #1 m by 1 m

## resample elevation raster to desired template 
#elev <- resample(elev, elev_template)

## OPTIONAL Save to file
#writeRaster(elev, filename='C:/geodata/elevation/90m_DEM.tif')

#Plot elevation hillshade
height_shade(raster_to_matrix(elev)) %>%
  plot_map()

r <- raster('C:/geodata/projectdata/90_005band1.tif')
g <- raster('C:/geodata/projectdata/90_005band2.tif')
b <- raster('C:/geodata/projectdata/90_005band3.tif')
...
#state boundary
#extent <- readOGR(dsn='C:/geodata/projectdata/Stateboundary.shp')
#Lacrosse County boundary 
extent <- readOGR(dsn='C:/geodata/projectdata/Lacrosseboundary.shp')
...
plot_3d(imagery_contrast, elev_matrix, windowsize = c(1100,900), zscale = 3, shadowdepth= -50,
        zoom=0.5, phi=45,theta=-45,fov=70, background = "#F2E1D0", shadowcolor = "#523E2B")
render_snapshot(title_text = "Lacrosse County, Wisconsin | Soil Color 5 cm | DEM: 10 m ",
